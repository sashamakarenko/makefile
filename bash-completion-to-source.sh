...
__makefile_maka_complete()
{
    ...
    if [[ -n "$cur" && t = "${cur:0:1}" ]]; then
        ...
        words="all release debug clean clean-tests clean-lcov lcov check recheck go gdb ldd gdb-debug gdb-release gdb-test- build-all-tests rebuild rebuild-all test- new-class generate-vscode show-var-"
    fi
